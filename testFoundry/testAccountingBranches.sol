...
contract TestAccountingBranches is testStarter, OptimismAddresses {
    ...
    function test_BypassDepositLimit() public {
        _dealWhale(USDC, alice, USDC_Whale, 1_000_000e6);

        vm.prank(owner);
        accountingManager.setDepositLimits(100_000e6, 150_000e6);

        vm.startPrank(alice);
        SafeERC20.forceApprove(IERC20(USDC), address(accountingManager), type(uint256).max);

        accountingManager.deposit(address(alice), 100_000e6, address(0));
        accountingManager.deposit(address(alice), 100_000e6, address(0));
        vm.stopPrank();
        ...
        assertGt(accountingManager.TVL(), accountingManager.depositLimitTotalAmount());
    }
