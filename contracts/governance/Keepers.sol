...
contract Keepers is EIP712, Ownable2Step {
    ...
    function updateOwners(address[] memory _owners, bool[] memory addOrRemove) public onlyOwner {
        ...
        require(numOwnersTemp <= 10 && threshold <= numOwnersTemp && threshold >= (numOwnersTemp + 1) / 2);
    ...
    function setThreshold(uint8 _threshold) public onlyOwner {
        require(_threshold <= numOwners && _threshold >= (numOwners + 1) / 2);
        threshold = _threshold;
    ...
    function execute(
        ...
        {
            bytes32 txInputHash =
                keccak256(abi.encode(TXTYPE_HASH, nonce, destination, keccak256(data), gasLimit, executor, deadline));
