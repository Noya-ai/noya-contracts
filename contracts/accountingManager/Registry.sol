...
contract PositionRegistry is AccessControl, IPositionRegistry, ReentrancyGuard {
    ...
    mapping(address => uint256) public dustLevel;
    ...
    function setDustLevel(address token, uint256 level) external onlyRole(MAINTAINER_ROLE) {
        dustLevel[token] = level;
    }
    ...
    */
        ...
        vaultExists(vaultId)
    {
        ...
        for (uint256 i = 0; i < _connectorAddresses.length; i++) {
            ...
            emit ConnectorAdded(vaultId, _connectorAddresses[i], _enableds[i]);
        }
    }
